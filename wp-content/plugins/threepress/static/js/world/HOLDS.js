...
const render_hold = async( event ) => {
	...
	try{
		...
		const { value, subtype, resource_type, state } = event

		console.log('render hold:', subtype, value )

		placeholder = Placeholder()

		// state
		STATE.set('holding')
		STATE.held_url = value
		STATE.held_subtype = subtype
...
const show_ui = ( state, value ) => {
	...
	if( state ){
		...
		// msg.innerHTML = 'placing: ...' + value.substr( value.length - 10 )
		// cancel.appendChild( msg )

		BROKER.publish('CHAT_ADD', {
			sender: 'system',
			chat_type: 'system',
			msg: 'placing: ...' + value.substr( value.length - 20 ),
		})
