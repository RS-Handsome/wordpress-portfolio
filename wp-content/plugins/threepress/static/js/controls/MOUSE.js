...
import RAYCASTER from './RAYCASTER.js?v=162'
...
const track_look = e => { // ( right click )
	...
	if( !STATE.first_person ){
		...
		CAMERA.position.y = Math.max( -1, Math.min( MAX_DIST, CAMERA.position.y ) )
		camera_look_home()
...
// 	if( tracking_look ) return console.log('tracking look abort')
	...
	switch( e.which ){
		case 1: // left
			...
			if( e.caller !== 'mouseout'){
				...
				debug_log('interactions-click', 'target mesh click: ', mesh )
...
function mouse_wheel( e ){
	...
	if( e.deltaY > 0 ){
		...
		if( STATE.first_person ) return console.log('already in first')
	...
	set_focal_length( current_cam_dist )
...
const max_focal = 65
let adjusted
const set_focal_length = dist => {
	adjusted = scry( dist || 1, 0, MAX_DIST, min_focal, max_focal )
	CAMERA.setFocalLength( adjusted )
}
...
function detect_object_clicked( e, bounds ){
	...
	while( !clicked && c < intersects.length ){
		clicked = get_clickable( intersects[c].object, 3 )
		c++
	}
...
// B)
function get_clickable( obj, depth ){
