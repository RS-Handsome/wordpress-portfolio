import BINDS from './BINDS.js?v=162'
	...
	if( !global_handled ){
		...
		if( !state ){
			...
			*/
			...
			case BINDS.world.remove_object:
				if( THREEPRESS.target_mesh ){
					BROKER.publish('SOCKET_SEND', {
						type: 'remove_object',
						uuid: THREEPRESS.target_mesh.userData.uuid,
					})					
				}
				break;
...
const handle_mouseout = event => {
	...
	if( mouseouts.includes( e.target.id ) ){
