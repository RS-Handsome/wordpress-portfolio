...
import '../inc/seedrandom.js?v=162'
import {
	Box3,
	Vector3,
...
const get_object_subtype = install => {

	// if( url ) return get_uri_type( url )

	if( !install ) return false

	if( install.value === 'tree' || install.value === 'cube' ) return 'readymade'

	const uritype = get_uri_type( install.value )
	if( uritype === 'image' || uritype === 'model') return 'upload'

	return console.log('unknown object subtype: ', install.value || uritype || install )
